...
$sectors = [];
$stmt = $pdo->query("SELECT DISTINCT sector_1 as sector FROM projects WHERE sector_1 IS NOT NULL AND sector_1 != '' UNION SELECT DISTINCT sector_2 as sector FROM projects WHERE sector_2 IS NOT NULL AND sector_2 != '' ORDER BY sector");
...
$municipalities = [];
$stmt = $pdo->query("SELECT DISTINCT municipality FROM projects WHERE municipality IS NOT NULL AND municipality != '' ORDER BY municipality");
while ($row = $stmt->fetch()) {
    $municipalities[] = $row['municipality'];
}

$programs = [];
$stmt = $pdo->query("SELECT DISTINCT programme FROM projects WHERE programme IS NOT NULL AND programme != '' ORDER BY programme");
while ($row = $stmt->fetch()) {
    $programs[] = $row['programme'];
}

$beneficiaries = [];
$stmt = $pdo->query("SELECT DISTINCT contracting_party FROM projects WHERE contracting_party IS NOT NULL AND contracting_party != '' ORDER BY contracting_party");
while ($row = $stmt->fetch()) {
